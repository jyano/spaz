...
g.connect("mongodb://localhost/brain",function(z){l('cn..')})

l('\n<m>');

ldM=function(ms,o){
    for(var m in ms){
        o[m]=g.model(m, g.Schema(ms[m]))};
    return o};
...
MS=r('connect-mongo')(e);
a.u(e.ss({secret:'xyz',store:MS({db:'brain'})}));
//a.u(cF());
...
_.e(gx,function(p){
    gA.push(
        "{src:'/png/"+
    P.b(p)+"',id:'"+ P.b(p).split('.')[0]+"'}")});a.l.mf=d("["+gA.join()+"]");
...
ssio=r('session.socket.io');


IO=new ssio(sio,MS,cP);
